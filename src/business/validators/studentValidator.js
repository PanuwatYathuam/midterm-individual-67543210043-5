class StudentValidator {
    ...
    validateStudentCode(code) {
        ...
        if (!codePattern.test(code)) throw new Error('Invalid student code format (must be 10 digits)');
        return true;
    }
    ...
    validateMajor(major) {
        ...
        if (!validMajors.includes(major)) throw new Error('Invalid major. Must be one of: CS, SE, IT, CE, DS');
        return true;
    }
    
    validateGPA(gpa) {
        if (gpa === undefined || gpa < 0 || gpa > 4.0) throw new Error('GPA must be between 0.0 and 4.0');
        return true;
    }
    
    validateStatus(status) {
        const validStatuses = ['active', 'graduated', 'suspended', 'withdrawn'];
        if (!validStatuses.includes(status)) throw new Error('Invalid status. Must be one of: active, graduated, suspended, withdrawn');
        return true;
    }
}
